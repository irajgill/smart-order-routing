...
contract KittenSwapAdapter is IDEXAdapter {
        ...
        returns (uint256 amountOut)
    {
        ...
        if (useStable) {
            amounts =
                router.swapExactTokensForTokensStable(amountIn, minAmountOut, path, msg.sender, block.timestamp + 300);
        } else {
            amounts = router.swapExactTokensForTokens(amountIn, minAmountOut, path, msg.sender, block.timestamp + 300);
        }
        ...
        returns (uint256 amountOut, uint256 gasEstimate)
    {
        ...
        uint256 baseOutput;
        if (useStable) {
            try router.getAmountsOutStable(amountIn, path) returns (uint256[] memory amounts) {
    ...
    function getPairInfo(address tokenA, address tokenB)
        external
        view
        returns (address pair, bool stable, uint256 reserve0, uint256 reserve1, uint256 totalSupply)
    {
    ...
    function getVeRewards(address account, address tokenA, address tokenB)
        external
        view
        returns (uint256 pendingRewards, uint256 rewardRate)
    {
    ...
    function calculateVolatileSwapOutput(uint256 amountIn, uint256 reserve0, uint256 reserve1)
        external
        pure
        returns (uint256 amountOut)
    {
    ...
    function estimateVeBonus(address, /*account*/ uint256 baseOutput) external pure returns (uint256 bonusOutput) {
