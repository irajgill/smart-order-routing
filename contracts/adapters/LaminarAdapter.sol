...
contract LaminarAdapter is IDEXAdapter {
    ...
    event CrossLayerSwap(
        address indexed tokenIn, address indexed tokenOut, uint256 amountIn, uint256 amountOut, string layer
    );

    event LiquidityAggregated(address indexed tokenA, address indexed tokenB, uint256 totalLiquidity, uint256 sources);
        ...
        returns (uint256 amountOut)
    {
        ...
        return laminarRouter.executeSwap(tokenIn, tokenOut, amountIn, minAmountOut, swapData);
    }
    ...
    {
        try laminarRouter.getOptimalSwap(tokenIn, tokenOut, amountIn) returns (uint256 optimal, bytes memory) {
            if (_shouldUseCrossLayer(tokenIn, tokenOut, amountIn)) {
    ...
    function _executeOnHyperCore(address tokenIn, address tokenOut, uint256 amountIn, uint256 minAmountOut)
        internal
        returns (uint256 amountOut)
    {
        ...
        amountOut = hyperCoreAdapter.executeHyperCoreSwap(tokenIn, tokenOut, amountIn, minAmountOut);
    ...
    function _shouldUseCrossLayer(address tokenIn, address tokenOut, uint256 amountIn) internal view returns (bool) {
        if (amountIn < 10000 * 10 ** 18) return false;

        try hyperCoreAdapter.getHyperCorePrice(tokenIn, tokenOut, amountIn) returns (uint256 corePrice, uint256) {
            try laminarRouter.getOptimalSwap(tokenIn, tokenOut, amountIn) returns (uint256 evmPrice, bytes memory) {
    ...
    function _getCrossLayerQuote(address tokenIn, address tokenOut, uint256 amountIn) internal view returns (uint256) {
        try hyperCoreAdapter.getHyperCorePrice(tokenIn, tokenOut, amountIn) returns (uint256 corePrice, uint256) {
            try laminarRouter.getOptimalSwap(tokenIn, tokenOut, amountIn) returns (uint256 evmPrice, bytes memory) {
    ...
    {
        (uint256 _totalLiquidity, uint256 _hyperEVMLiquidity, uint256 _hyperCoreLiquidity, address[] memory _sources) =
            laminarRouter.getAggregatedLiquidity(tokenA, tokenB);
    ...
    function getCrossLayerArbitrageOpportunity(address tokenA, address tokenB, uint256 amountIn)
        external
        view
        returns (bool hasOpportunity, uint256 expectedProfit, string memory optimalLayer)
    {
        try hyperCoreAdapter.getHyperCorePrice(tokenA, tokenB, amountIn) returns (uint256 corePrice, uint256) {
            try laminarRouter.getOptimalSwap(tokenA, tokenB, amountIn) returns (uint256 evmPrice, bytes memory) {
    ...
    function updatePairLiquidity(address tokenA, address tokenB, uint256 liquidity) external {
    ...
    function calculateAggregatedPriceImpact(address tokenIn, address tokenOut, uint256 amountIn)
        external
