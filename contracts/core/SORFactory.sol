...
contract SORFactory is Ownable, Pausable, ReentrancyGuard {
    ...
    event RouterCreated(
        address indexed router, address indexed creator, address indexed priceOracle, uint256 routerId, string name
    );
    ...
    constructor(uint256 _creationFee, address _feeRecipient, address _defaultPriceOracle) {
    ...
    function createRouter(address priceOracle, address feeCollector, string calldata name, string calldata description)
        external
        payable
        whenNotPaused
        nonReentrant
        returns (address router)
    {
        ...
        require(_getUserRouterCount(msg.sender) < maxRoutersPerUser, "SORFactory: Router limit exceeded");
    ...
    function createAdvancedRouter(
        ...
        router = this.createRouter{ value: msg.value }(priceOracle, feeCollector, name, description);
    ...
    function _deployProxyRouter(address priceOracle, address feeCollector) internal returns (address) {
    ...
    function getRouterStats(address router)
        external
        view
        validRouter(router)
        returns (
        ...
        )
    {
        ...
        return (info.creator, info.createdAt, info.totalVolume, info.verified, info.name, info.description);
    }
    ...
    function getFactoryStats()
        external
        view
        returns (uint256 totalRouters, uint256 verifiedRouters, uint256 totalVolume, uint256 totalFees)
    {
        uint256 verified = 0;
        uint256 volume = 0;
        ...
        return (allRouters.length, verified, volume, address(this).balance);
    }
