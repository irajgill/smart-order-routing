...
contract SORManager is Ownable, Pausable, ReentrancyGuard {
    ...
    function addDEXAdapter(string calldata name, address adapter, string calldata version, address maintainer)
        external
        onlyOwner
        validDEXName(name)
    {
    ...
    function updateDEXAdapter(string calldata name, address newAdapter, string calldata newVersion)
        external
        onlyOwner
        validDEXName(name)
    {
    ...
    function addSupportedToken(address token, uint256 decimals) external onlyOwner {
        ...
        this.addSupportedToken(token, uint8(decimals), "UNKNOWN", "Unknown Token", address(0));
    }
    ...
    // Router Authorization
    function authorizeRouter(address router, RouterPermissions calldata permissions) external onlyOwner {
    ...
    function updateRouterPermissions(address router, RouterPermissions calldata newPermissions) external onlyOwner {
        require(authorizedRouters[router], "SORManager: ROUTER_NOT_AUTHORIZED");
    ...
    // Volume tracking
    function recordSwap(address router, address tokenIn, address tokenOut, uint256 volumeUSD)
        external
        onlyAuthorizedRouter
    {
        ...
        require(perms.dailyVolumeUsed + volumeUSD <= perms.maxDailyVolume, "SORManager: DAILY_VOLUME_EXCEEDED");
    ...
    function getAllDEXAdapters()
        external
        view
        returns (string[] memory names, address[] memory adapters, bool[] memory enabled)
    {
        names = dexNames;
    ...
    function getEnabledDEXAdapters() external pure returns (string[] memory names, address[] memory adapters) {
