...
interface IPriceOracle {
    ...
    event PriceUpdated(
        address indexed tokenA, address indexed tokenB, uint256 price, uint256 timestamp, uint256 confidence
    );

    event OracleSourceAdded(address indexed source, string name, uint256 weight);
     ...
     */
    function getTWAP(address tokenA, address tokenB, uint256 period) external view returns (uint256 price);
     ...
     */
    function updatePrice(address tokenA, address tokenB, uint256 price) external;
     ...
     */
    function getPriceWithBounds(address tokenA, address tokenB)
        external
        view
