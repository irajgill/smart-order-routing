...
library RouteCalculator {
    ...
    function calculateOptimalRoute(RouteParams memory params, address[] memory availableAdapters)
        internal
        pure
        returns (RouteResult memory)
    {
    ...
    function calculatePriceImpact(uint256 amountIn, uint256 liquidity) internal pure returns (uint256) {
    ...
    function calculateOutputAmount(uint256 amountIn, uint256 reserveIn, uint256 reserveOut, uint256 fee)
        internal
        pure
        returns (uint256)
    {
    ...
    function calculateRouteScore(uint256 expectedOutput, uint256 priceImpact, uint256 gasEstimate)
        internal
        pure
        returns (uint256)
    {
        ...
        uint256 score = outputScore > (impactPenalty + gasPenalty) ? outputScore - impactPenalty - gasPenalty : 0;
    ...
    function optimizeMultiHopRoute(
        ...
        for (uint256 i = 0; i < tokens.length - 1; i++) {
            currentAmount = calculateOutputAmount(currentAmount, reserves[i * 2], reserves[i * 2 + 1], fees[i]);
        }
    ...
    function findBestSplitPercentages(
        ...
        for (uint256 i = 0; i < routeCount; i++) {
            ...
            scores[i] = routeGasEstimates[i] > 0 ? (routeOutputs[i] * 1e18) / routeGasEstimates[i] : 0;
            totalScore += scores[i];
        }
