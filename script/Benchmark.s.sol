...
contract BenchmarkScript is Script {
    ...
    function run() public {
        ...
        string[4] memory testNames = [
            "Small Trade (1K HYPE)",
            "Medium Trade (10K HYPE)",
            "Large Trade (100K HYPE)",
            "Whale Trade (1M HYPE)"
        ];
        ...
        for (uint256 i = 0; i < testAmounts.length; i++) {
            ...
            try sorRouter.getSwapQuote(hypeToken, usdcToken, testAmounts[i]) 
            returns (uint256 amountOut, uint256 gasEstimate, SORRouter.SplitRoute[] memory routes) {
