...
contract MockPriceOracle is IPriceOracle {
    ...
    function getPrice(address tokenA, address tokenB) external view override returns (PriceData memory) {
        ...
        if (priceData.price == 0) {
            return PriceData({ price: 1e18, timestamp: block.timestamp, confidence: 95 });
        }

        return priceData;
    }

    function getTWAP(address tokenA, address tokenB, uint256 /*period*/ )
        external
        view
        override
    ...
    function updatePrice(address tokenA, address tokenB, uint256 price) external override {
        bytes32 key = keccak256(abi.encodePacked(tokenA, tokenB));
        prices[key] = PriceData({ price: price, timestamp: block.timestamp, confidence: 95 });
